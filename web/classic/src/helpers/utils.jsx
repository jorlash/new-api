...
import { withRouterBasename } from './basePath';
...
export function getSystemName() {
  ...
  if (!system_name) return 'Router Hub';
  return system_name;
}

export function getLogo() {
  let logo = localStorage.getItem('logo');
  if (!logo) return withRouterBasename('/logo.png');
  return logo;
}
    ...
    if (error.name === 'AxiosError') {
      switch (error.response.status) {
        case 401:
          ...
          window.location.href = withRouterBasename('/login?expired=true');
          break;
        case 429:
